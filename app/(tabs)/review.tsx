...
export default function ReviewScreen() {
  ...
  const fetchReviewedQuestions = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // Get latest quiz_sessions for user and selected version (limit to last 50 sessions)
      ...
      // Map to UI format
      const reviewed = answers.map((a: any, idx: number) => ({
        id: a.id,
